...
cv::Mat& bug_1(cv::Mat image)
{

}

cv::Mat& bug_2(cv::Mat image)
{
...
// cp_image.at<cv::Vec3b>(cv::Point2i(x,y))
...
cv::Mat& rotate(cv::Mat &image, cv::Mat &output, int rotation=0)
{
    ...
    for(int x = 0; x < image.rows ; x++)
    {
        ...
        {
			cp_image.at<cv::Vec3b>(cv::Point2i(image.rows-1 -y,x)) = image.at<cv::Vec3b>(cv::Point2i(x,y));
